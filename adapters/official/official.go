...
func (a *Adapter) Parse(data []byte) (*adapters.IngestionResult, error) {
	...
	for _, rec := range records {
		...
		for _, src := range rec.Sources {
			...
			if createdAt.IsZero() || publication.Before(createdAt) { createdAt = publication }
			if effective.After(updatedAt) { updatedAt = effective }
			if effective.After(lastChange) { lastChange = effective }
		}
		...
		if rec.Latitude != nil { project.Latitude = *rec.Latitude }
		if rec.Longitude != nil { project.Longitude = *rec.Longitude }
		if rec.CapexCAD != nil { project.CapexCAD = *rec.CapexCAD }
		...
		for _, event := range rec.Events {
			...
			if !ok { return nil, fmt.Errorf("event %q references unknown source %q", event.ExternalID, event.SourceID) }
		...
		for _, item := range rec.CapitalItems {
			...
			if !ok { return nil, fmt.Errorf("capital item %q references unknown source %q", item.ExternalID, item.SourceID) }
...
func validateRecord(rec projectRecord) error {
	...
	if rec.Stage == "" { return fmt.Errorf("official record %q has no explicit stage state", rec.ExternalID) }
	...
	if rec.CapexCAD != nil && *rec.CapexCAD < 0 { return fmt.Errorf("official record %q has negative capex", rec.ExternalID) }
	for _, src := range rec.Sources {
...
func parseTime(value, field, id string) (time.Time, error) {
	...
	if err != nil { return time.Time{}, fmt.Errorf("%s for %q: %w", field, id, err) }
	return t, nil
}
